...
const IslamicTools = () => {
  ...
  useEffect(() => {
    ...
    const calculateNextPrayer = () => {
      ...
      const upcoming = times[0] || null;

      if (upcoming) {
        const countdownInSeconds = Math.max(
          0,
          Math.floor((upcoming.time - now) / 1000)
        );
  ...
  if (!prayerTimes || !date || qiblaDirection === null) {
    return (
      ...
      <Card className="w-full max-w-md shadow-md mb-4">
        <CardContent>
          ...
          <div className="flex flex-col items-center mb-4">
            ...
            >
              <div className="h-2 w-12 bg-red-500 absolute top-1/2 transform -translate-y-1/2"></div>
